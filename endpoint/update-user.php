...
try {
    ...
    if (empty($nameExist)) {
        ...
        $updateStmt = $conn->prepare("UPDATE `tbl_user` SET `first_name` = :first_name, `last_name` = :last_name, `contact_number` = :contact_number, `email` = :email, `address` = :address, `state` = :state, `username` = :username, `password` = :password WHERE `tbl_user_id` = :userID");
        ...
        $updateStmt->bindParam(':state', $updatestate, PDO::PARAM_STR);
